                ...
                <div class="col-lg-12">
                    <div class="card border-top border-0 border-4 border-primary">
                        <div class="card-body p-4">
                            <form id="myForm" action="{{ route('update.specialization') }}" method="post" enctype="multipart/form-data">
                                @csrf
                                <input type="hidden" name="id" value="{{ $specialization_id->id }}">
                                        ...
                                        <div class="row">
                                                ...
                                                <select class="js-example-basic-single mb-3 form-groups" name="strem_id">
                                                    <option>Select Strem </option>
                                                        @foreach ($strem as $item )
                                                            <option value="{{ $item->id }}" {{ $item->id == $specialization_id->strem_id ? 'selected' : ''  }}>{{ $item->strem_name }}</option>
                                                        @endforeach
                                                </select>
                                            </div>
                                            <!--end col-->
                                                ...
                                                <select class="js-example-basic-single mb-3 form-groups" name="substrem_id">
                                                    <option>Select Substrem </option>
                                                        @foreach ($substrem as $item )
                                                             <option value="{{ $item->id }}" {{ $item->id  == $specialization_id->substrem_id ? 'selected' : ''}}>{{ $item->substrem_name }}</option>
                                                        @endforeach
                                                </select>
                                            </div>
                                            <!--end col-->
                                                ...
                                                <select class="js-example-basic-single mb-3 form-groups" name="course_id">
                                                    <option>Select Course </option>
                                                        @foreach ($course as $item )
                                                            <option value="{{ $item->id }}" {{ $item->id == $specialization_id->course_id ? 'selected' : ''  }}>{{ $item->course_name }}</option>
                                                       @endforeach
                                                </select>
                                            </div>
                                            <!--end col-->
                                            <div class="col-lg-4">
                                                <div class="mb-3 form-groups">
                                                    ...
                                                    <input type="text" name="specialization_name" class="form-control" id="specializationsnameInput" placeholder="Enter your specialization name" value="{{ $specialization_id->specialization_name }}">
                                                </div>
                                            </div>
                                            <!--end col-->
                                    </div>
