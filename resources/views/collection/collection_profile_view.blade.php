                    ...
                    <div class="card-body p-4">
                        <div class="text-center">
                            <div class="profile-user position-relative d-inline-block mx-auto  mb-4">
                                <img src="{{ (!empty($collectionData->photo)) ? url($collectionData->photo) : url('upload/no_image.jpg') }}" class="rounded-circle avatar-xl img-thumbnail user-profile-image" alt="user-profile-image">
                    ...
                    <div class="card-body p-4">
                        <div class="tab-content">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-3 form-groups">
                                                ...
                                                <input type="tel" name="phone" class="form-control" id="phonenumberInput" maxlength="10" placeholder="Enter your phone number" value="{{ $collectionData->phone }}" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*?)\..*/g, '$1');">
                                        ...
                                        <div class="col-lg-3">
                                            <div class="mb-3">
                                                ...
                                                <img id="showImage" src="{{ (!empty($collectionData->photo)) ? url($collectionData->photo) : url('upload/no_image.jpg') }}" alt="image" style="width:100px; height:100px;">
                            ...
                            <div class="tab-pane" id="locationDetails" role="tabpanel">
                                <form id="myForm" action="{{ route('collection.location.store') }}" method="post">
                                    @csrf
                                    <div class="row">
                                        <div class="col-lg-3">
                                            ...
                                            <select class="js-example-basic-single mb-3" name="country_id">
                                                <option>Select Country</option>
                                                    @foreach ($countries as $item)
                                                        <option value="{{ $item->id }}" {{ $item->id == $collectionData->collection->country_id ? 'selected' : '' }}>{{ $item->country_name }}</option>
                                                    @endforeach
                                            ...
                                            <select class="js-example-basic-single mb-3" name="state_id">
                                                <option>Select State</option>
                                                    @foreach ($states as $item)
                                                        <option value="{{ $item->id }}" {{ $item->id == $collectionData->collection->state_id ? 'selected' : '' }}>{{ $item->state_name }}</option>
                                                    @endforeach
                                            </select>
                                        </div>
                                        <!--end col-->
                                            ...
                                            <select class="js-example-basic-single mb-3" name="city_id">
                                                <option>Select City</option>
                                                    @foreach ($cities as $item)
                                                        <option value="{{ $item->id }}" {{ $item->id == $collectionData->collection->city_id ? 'selected' : '' }}>{{ $item->city_name }}</option>
                                                    @endforeach
                                            </select>
                                        </div>
                                        <!--end col-->
                                        <div class="col-lg-3">
                                            <div class="mb-3 form-groups">
                                                ...
                                                <input type="text" class="form-control" name="locationname" id="nameInput" placeholder="Enter Location" value="{{ $collectionData->collection->locationname }}">
                                            </div>
                                        </div>
                                        <!--end col-->
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('select[name="country_id"]').on('change', function(){
            ...
            if (country_id) {
                $.ajax({
                    url: "{{ url('/collectionprofilestate/ajax') }}/"+country_id,
                    type: "GET",
                    dataType:"json",
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        $('select[name="state_id"]').on('change', function(){
            ...
            if (state_id) {
                $.ajax({
                    url: "{{ url('/collectionprofilecity/ajax') }}/"+state_id,
                    type: "GET",
                    dataType:"json",
                    success:function(data){
...
<script type="text/javascript">
    $(document).ready(function (){
        ...
        $.ajax({
            type: "POST",
            url: "{{ route('collectioncheck.phone') }}", // Change this to the route that checks the phone number
            data: {
                phone: value,
